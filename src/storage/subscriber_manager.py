...
# 베이스 경로 (src/storage/ -> 프로젝트 루트 -> data)
_DATA_DIR = Path(__file__).parent.parent.parent / "data"

def _get_path() -> Path:
    """구독자 파일 경로 반환 (사전규격 전용)"""
    return _DATA_DIR / "subscribers_prebid.json"


def load_subscribers() -> set[str]:
    """subscribers_prebid.json에서 구독자 Chat ID 목록을 로드합니다.
...
def save_subscribers(subscribers: set[str]) -> None:
    """구독자 목록을 subscribers_prebid.json에 저장합니다.
...
def add_subscriber(chat_id: str) -> bool:
    ...
    logger.info(f"새로운 구독자 자동 등록: {chat_id}")
    return True
